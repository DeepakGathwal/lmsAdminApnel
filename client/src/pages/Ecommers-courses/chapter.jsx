...
const EChapter = () => {
  ...
  const allData = async (limit) => {
    ...
    const data = await allCourseChapter(path, givenLimit, currentPage)
    setState(data)
  }
  ...
  const ConfirmBox = async (id) => {
    ...
    if (value) {
      const deleteMember = await deleteCourseChapter(path, id)
  ...
  const handelCreateAndUpdate = async (e) => {
    ...
    if (editshow.id > 0) {
      ...
      value = await editCourseChapter(path, editshow, removeSameData)
    } else {
      const courcesid = selected && await selected.map((el) => el.value)
      value = await createCourseChapter(path, editshow, courcesid)
    }
    e.preventDefault()
  ...
  return (
    <div className="containers">
      <div className="page">
        <h3 className='heading mb-4'>Course Chapters</h3>
      ...
      <Modal show={show} onHide={() => handleClose(setShow, setEditShow)}>
        ...
        <Modal.Header closeButton>
          <Modal.Title>Add Course Chapters</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <form className='batchesform' action="" onSubmit={handelCreateAndUpdate}>
              ...
              <div className="form-group">
                <label htmlFor="">Chapter
                  <input type="text" name="chapter" id="" value={editshow.chapter} className="form-control" onChange={handelChange} required />
                </label>
              </div>
