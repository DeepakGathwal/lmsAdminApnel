...
const Prerequisite = () => {
  ...
  const ConfirmBox = async (id) => {
    ...
    if (value) {
      const deleteMember = await deleteCourcePrerequisite(path, id)
  ...
  const handelCreateAndUpdate = async (e) => {
    ...
    if (editshow.id > 0) {
      ...
      value = await editCourcePrerequisite(path, editshow, removeSameData)
    } else {
      const courcesid = selected && await selected.map((el) => el.value)

      value = await createCourcePrerequisite(path, editshow, courcesid)
    }
    e.preventDefault()
  ...
  return (
    <div className="containers">
      <div className="page">
        <h3 className='heading mb-4'>Course Prerequisite</h3>
      ...
      <Modal show={show} onHide={() => handleClose(setShow, setEditShow)}>
        ...
        <Modal.Header closeButton>
          <Modal.Title>Courses Prerequisite</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <form className='batchesform' action="" onSubmit={handelCreateAndUpdate}>
              ...
              <div className="form-group">
                <label htmlFor="">Prerequisite
                  <input type="text" name="requirement" id="" value={editshow.requirement} className="form-control" onChange={handelChange} required />
                </label>
              </div>
              <input type="submit" value="Add Requirement" className=' btn btn-primary mt-4 mb-2 btn-create' required />
