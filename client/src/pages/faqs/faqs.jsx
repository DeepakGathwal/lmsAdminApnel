...
let optionArray = []
  ...
  const courcesList = async() =>{
    const {data} = await faqsPages(path)
   
    if(data){
      data.map((el) => {
      optionArray.push({label : el.name, value : el.name })
      })
 ...
 const handleClose = ( ) => {
  ...
  const ConfirmBox = async (id) => {
    const value = window.confirm("Are you Sure want to delete");
     if (value) {
      const deleteMember = await deleteFaqs(path, id)
  ...
  const handelCreateAndUpdate = async (e) => {
    ...
    if (editshow.id > 0) {
      ...
      value = await editFaqs(path, editshow, removeSameData)
    } else {
      const courcesid = selected && await selected.map((el) => el.value)
      
      value = await createFaqs(path, editshow,courcesid )
    }
    e.preventDefault()
