...
exports.chartApi = catchAsyncError(async (req, res) => {
    ...
    if (!redisdata) {
        const api = ` SELECT months.month, COALESCE(course_counts.courses, 0) AS ecommers_course, COALESCE(user_counts.users, 0) AS users, COALESCE(testimonial_counts.testimonials, 0) AS website_course, COALESCE(video_counts.videos, 0) AS videos
    FROM 
        (
    ...
    LEFT JOIN 
        (
            ...
            FROM jtc_ecommers_videos WHERE deleted_by = '0'
            GROUP BY DATE_FORMAT(created_at, '%Y-%m')
