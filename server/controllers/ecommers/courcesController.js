...
exports.deleteCource = catchAsyncError(async (req, res) => {
  const { permissions, user } = await req
  if (permissions.can_delete == 0) return res.status(206).json({ message: "Permission Denied to Delete Course", status: false });
  const { id } = await req.params
  if (!id) return res.status(200).json({ message: "Comapny not found", success: false })
  const query = `update  jtc_ecommers_courses SET deleted_by = ${user}, deleted_at = current_timestamp()`
  const executeAddPoint = await executeQuery(query)
...
exports.editChapter = catchAsyncError(async (req, res) => {
  const { permissions, user } = await req
  if (permissions.can_edit == 0) return res.status(206).json({ message: "Permission Denied to Edit Course Chapter", status: false });
  const { id } = await req.params
...
exports.removeChapter = catchAsyncError(async (req, res) => {
  const { permissions, user } = await req
  if (permissions.can_delete == 0) return res.status(206).json({ message: "Permission Denied to Delete Course Chapter", status: false });
  const { id } = await req.params
