...
exports.addLearn = catchAsyncError(async (req, res) => {
  ...
  exports.usersPayments = catchAsyncError(async(req,res) =>{
    ...
    const query = `Select users.name,users.email, course.name as course, course.course_link as link,payment.id,payment.verify,payment.amount,payment.currency,payment.payment_id,payment.payment_signature,payment.order_id ,payment.done_time,payment.order_at from jtc_ecommers_course_payment as payment Left Join	jtc_ecommers_courses as course On payment.course = course.id And course.deleted_by = '0' Left Join jtc_ecommers_users as users On users.id = payment.user and users.deleted_by = '0'`
